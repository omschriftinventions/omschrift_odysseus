...
export const THEMES = {
  ...
  omssetu:    { bg:'#f6f8ff', fg:'#0c0f1e', panel:'#ffffff', border:'#c5c9dc', red:'#4f5de8',
                advanced: {
                  userBubbleBg: '#eff2fc',
                  aiBubbleBg: '#ffffff',
                  bubbleBorder: '#c5c9dc',
...
function _updateFavicon(fg) {
  ...
  apple.href = routeShape ? href : '/static/icons/icon-192.png';
}
...
export function applyBgPattern(pattern) {
  ...
  document.querySelectorAll('#synapse-canvas, #rain-canvas, #constellations-canvas, #perlin-flow-canvas, #petals-canvas, #sparkles-canvas, #embers-canvas, #fleet-bg, #fleet-cursor-halo').forEach(c => c.remove());
  if (window._fleetBgCleanup) { window._fleetBgCleanup(); window._fleetBgCleanup = null; }
...
export function initThemeUI() {
  ...
  grid.innerHTML = Object.entries(THEMES).map(([name, c]) => `
    <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}">
      ...
      ${name === 'dark' ? 'original' : (name === 'gpt' ? 'GPT' : (name === 'omssetu' ? 'Oms Setu' : name))}
    </div>
  `).join('');
...
// ── Fleet / Oms Setu — ambient orbs, grid spotlight, particle network ──
function _initFleet() {
  if (document.getElementById('fleet-bg')) return;

  const wrap = document.createElement('div');
  wrap.id = 'fleet-bg';
  wrap.className = 'fleet-bg-field';
  wrap.setAttribute('aria-hidden', 'true');
  wrap.innerHTML = [
    '<div class="fleet-b-base"></div>',
    '<div class="fleet-beam"></div>',
    '<div class="fleet-par fleet-p1"><div class="fleet-orb fleet-o1"></div></div>',
    '<div class="fleet-par fleet-p2"><div class="fleet-orb fleet-o2"></div></div>',
    '<div class="fleet-par fleet-p3"><div class="fleet-orb fleet-o3"></div></div>',
    '<div class="fleet-grid-base"></div>',
    '<div class="fleet-grid"></div>',
    '<div class="fleet-spot"></div>',
    '<div class="fleet-spot-core"></div>',
    '<canvas class="fleet-net" id="fleet-net-canvas"></canvas>',
    '<div class="fleet-grain"></div>',
    '<div class="fleet-vign"></div>',
  ].join('');
  document.body.prepend(wrap);

  const halo = document.createElement('div');
  halo.id = 'fleet-cursor-halo';
  halo.className = 'fleet-cursor-halo';
  ...
  const tickCursor = () => {
    if (!document.body.classList.contains('bg-pattern-fleet')) return;
    mx += (tx - mx) * 0.12;
    my += (ty - my) * 0.12;
    root.style.setProperty('--fleet-mx', mx + 'px');
    root.style.setProperty('--fleet-my', my + 'px');
    root.style.setProperty('--fleet-px', ((tx / window.innerWidth) * 2 - 1).toFixed(3));
    root.style.setProperty('--fleet-py', ((ty / window.innerHeight) * 2 - 1).toFixed(3));
  ...
  const canvas = document.getElementById('fleet-net-canvas');
  const ctx = canvas.getContext('2d');
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  let w = 0;
  let h = 0;
  ...
  function fleetAccentRgb() {
    const s = getComputedStyle(document.documentElement);
    const hex = (s.getPropertyValue('--bg-effect-color').trim()
      || s.getPropertyValue('--red').trim()
      || '#4f5de8').replace('#', '');
    if (hex.length !== 6) return { r: 79, g: 93, b: 232 };
    return {
  ...
  function initParticles() {
    ...
    const n = Math.max(60, Math.min(140, target));
  ...
  const onNetMove = (e) => { mouse.x = e.clientX; mouse.y = e.clientY; mouse.on = true; };
  const onNetLeave = () => { mouse.on = false; };
  ...
  function drawNet() {
    ...
    const mxv = mouse.x - rect.left;
    const myv = mouse.y - rect.top;
    const maxD = 150;
    ctx.lineWidth = 0.55;
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < maxD) {
          const alpha = (1 - dist / maxD) * 0.32 * inten;
    ...
    if (mouse.on) {
      ...
      for (const p of particles) {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < maxC) {
          const alpha = (1 - dist / maxC) * 0.5 * inten;
    ...
    for (const p of particles) {
      ...
      ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${(0.42 * inten).toFixed(3)})`;
      ctx.fill();
    }
  }

  const onResize = () => { resizeNet(); initParticles(); };
  resizeNet();
  initParticles();
  drawNet();
  window.addEventListener('resize', onResize);
  window.addEventListener('pointermove', onNetMove, { passive: true });
  window.addEventListener('pointerleave', onNetLeave);

  window._fleetBgCleanup = () => {
    cancelAnimationFrame(cursorRaf);
    cancelAnimationFrame(netRaf);
    window.removeEventListener('pointermove', onPointerMove);
    window.removeEventListener('pointerleave', onPointerLeave);
    window.removeEventListener('resize', onResize);
    window.removeEventListener('pointermove', onNetMove);
    window.removeEventListener('pointerleave', onNetLeave);
    wrap.remove();
    halo.remove();
    root.style.removeProperty('--fleet-mx');
    root.style.removeProperty('--fleet-my');
    root.style.removeProperty('--fleet-px');
    root.style.removeProperty('--fleet-py');
  };
}
